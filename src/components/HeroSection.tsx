...
const HeroSection = () => {
  ...
  const sliderImages = [
    ...
    },
    {
      ...
      description:
        "Assublue space with contracted rates to major trade routes .",
      gradient: "",
    },
  ];
  ...
  return (
    <>
          ...
          >
            <div className="grid grid-cols-4 gap-1 sm:gap-2">
              {portalLinks.map((link, index) => {
                ...
                } else if ((link as any).onClick) {
                  return (
                    <button key={index} onClick={(link as any).onClick} className="w-full">
                      {ButtonContent}
                    </button>
                  );
                } else {
                  return (
                    <Link to={(link as any).url} key={index} className="w-full">
                      {ButtonContent}
                    </Link>
                  );
                }
              ...
              <div className="p-6 overflow-y-auto">
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[
                      { src: "/GC_demo_1.mp4", label: "Getting Started", duration: "5:32" },
                      { src: "/GC_promo_2.mp4", label: "Advanced Features", duration: "7:45" },
                    ].map((video, i) => (
                      <div
                        key={i}
      ...
      {/* ===== BELOW THE HERO: ONLY VIDEO SECTION ===== */}
      <section id="intro-video" className="bg-black">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
          <div className="aspect-[16/9] w-full overflow-hidden rounded-2xl shadow-2xl border border-white/10">
            <video
              className="w-full h-full object-cover"
              src="/hero5.mp4"        /* <- replace with your video path */
              controls
              playsInline
              poster="/video-cover.jpg"     /* optional thumbnail */
            />
          </div>
        </div>
      </section>
