...
const Navigation = () => {
  ...
  return <header className="fixed top-0 left-0 right-0 w-full z-50 shadow-md backdrop-blur supports-[backdrop-filter]:backdrop-blur transition-all duration-300 bg-slate-50">
      {/* Main Nav Bar */}
      <div className="container mx-auto px-3 sm:px-4 md:px-6 py-2 sm:py-4 lg:py-[18px]">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center">
            <img src="/lovable-uploads/a44481e1-bf8c-43ab-b259-b833b252e1ed.png" alt="Amass Middle East Shipping Services LLC" className="h-8 sm:h-12 lg:h-14 w-auto object-contain" />
          </div>

          {/* Desktop Navigation */}
          <div className="hidden lg:flex items-center gap-5">
            <Link to={getNavLink("/home")} className={`nav-link font-medium text-sm xl:text-base hover:text-amass-blue transition-colors ${isActive(getNavLink("/home")) || currentCountry.code === "SG" && isActive("/") ? "text-amass-blue" : "text-black"}`}>
              Home
            </Link>

            {/* Info Dropdown */}
            <DropdownMenu open={isCompanyDropdownOpen} onOpenChange={setIsCompanyDropdownOpen}>
              <DropdownMenuTrigger className={`nav-link font-medium text-sm xl:text-base flex items-center gap-1 hover:text-amass-blue transition-colors ${isCompanyLinkActive() ? "text-amass-blue" : "text-black"}`}>
            ...
            <Link to={getNavLink("/services")} className={`nav-link font-medium text-sm xl:text-base hover:text-amass-blue transition-colors ${isActive(getNavLink("/services")) ? "text-amass-blue" : "text-black"}`}>
              Services
            </Link>

            <Link to={getNavLink("/blog")} className={`nav-link font-medium text-sm xl:text-base hover:text-amass-blue transition-colors ${isActive(getNavLink("/blog")) || isActive(getNavLink("/blogs")) ? "text-amass-blue" : "text-black"}`}>
              Blogs
            </Link>
          ...
          <div className="hidden md:flex items-center gap-2 lg:gap-3">
            ...
            <div className="ml-1 flex items-center gap-2">
              ...
              href,
              Icon
            }) => <a key={name} href={href} target="_blank" rel="noopener noreferrer" aria-label={name} className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-gray-200 hover:border-amass-blue hover:text-amass-blue text-gray-700 transition-all">
      ...
      {isMenuOpen && <div className="lg:hidden absolute top-full left-0 right-0 bg-white py-4 shadow-md animate-fade-in border-t max-h-[calc(100vh-80px)] overflow-y-auto">
          <div className="container mx-auto px-4">
            ...
            path: "/contact"
              ...
              href,
              Icon
            }) => <a key={name} href={href} target="_blank" rel="noopener noreferrer" aria-label={name} className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 hover:border-amass-blue hover:text-amass-blue text-gray-700 transition-all">
